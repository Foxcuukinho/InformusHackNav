A Project for HackNAV 2026, Informus Team.

The Code.Canvas (flux) is not visible on GitHub. Download the archive and use an app like Obsidian to view it.

Changelog

28/03

The code, flow and idea were too complex — it doesn't need to be all that. Starting from the commit "Simplify The System", the flow is much more simplified. The CSS and HTML won't be overly complex either.

- Sorry for the large number of commits, i am a new in github.
